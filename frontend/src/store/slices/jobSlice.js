...
export const fetchJobs=(city,niche,searchKeyword="")=>async(dispatch)=>{
    try{
        ...
        let link=`${process.env.VITE_BACKEND_URL}/api/v1/job/getall?`
        let queryParams=[];
        if(searchKeyword){
...
export const fetchSingleJob=(jobId)=>async(dispatch)=>{
    ...
    try {
        const response=await axios.get(`${process.env.VITE_BACKEND_URL}/api/v1/job/get/${jobId}`,{withCredentials:true})
...
export const postJob=(data)=>async(dispatch)=>{
    ...
    try {
        const response=await axios.post(`${process.env.VITE_BACKEND_URL}/api/v1/job/post`,data,{withCredentials:true,headers:{"Content-Type":"application/json"}})
...
export const getMyJobs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${process.env.VITE_BACKEND_URL}/api/v1/job/getmyjobs`,
...
export const deleteJob=(id)=>async(dispatch)=>{
    ...
    try{
        const response=await axios.delete(`${process.env.VITE_BACKEND_URL}/api/v1/job/delete/${id}`,{withCredentials:true})
